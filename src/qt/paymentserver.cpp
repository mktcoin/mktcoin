...
const int MKTCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString MKTCOIN_IPC_PREFIX("mktcoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(MKTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // mktcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseMKTcoinURI(arg, &r) && !r.address.isEmpty())
            {
                CMKTcoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(MKTCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(MKTCOIN_IPC_CONNECT_TIMEOUT);
...
//
// OSX-specific way of handling mktcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(MKTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // mktcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseMKTcoinURI(s, &recipient))
            {
                CMKTcoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CMKTcoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom mktcoin addresses are not supported
        ...
        // MKTcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(MKTcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
