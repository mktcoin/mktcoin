...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"address\"    (string) The new mktcoin address\n"
            "\nExamples:\n"
    ...
    return CMKTcoinAddress(keyID).ToString();
}


CMKTcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CMKTcoinAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current MKTcoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account mktcoin address\n"
            "\nExamples:\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mktcoin address to be associated with an account.\n"
    ...
    CMKTcoinAddress address(request.params[0].get_str());
    if (!address.IsValid())
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mktcoin address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "  \"address\"         (string) a mktcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CMKTcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CMKTcoinAddress& address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The mktcoin address to send to.\n"
            ...
            "                             The recipient will receive less mktcoins than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The mktcoin address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CMKTcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CMKTcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CMKTcoinAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw runtime_error(
            ...
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mktcoin address to use for the private key.\n"
    ...
    CMKTcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mktcoin address for transactions.\n"
    ...
    // MKTcoin address
    CMKTcoinAddress address = CMKTcoinAddress(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MKTcoin address");
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a mktcoin address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The mktcoin address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(request.params[0]);
    CMKTcoinAddress address(request.params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MKTcoin address");
    CAmount nAmount = AmountFromValue(request.params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The mktcoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less mktcoins than you enter in their corresponding amount field.\n"
    ...
    set<CMKTcoinAddress> setAddress;
    vector<CRecipient> vecSend;
    ...
    {
        CMKTcoinAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid MKTcoin address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "2. \"keys\"         (string, required) A json array of mktcoin addresses or hex-encoded public keys\n"
            "     [\n"
            ...
            "\nResult:\n"
            "\"address\"         (string) A mktcoin address associated with the keys.\n"
    ...
    return CMKTcoinAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    return CMKTcoinAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CMKTcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CMKTcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CMKTcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The mktcoin address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The mktcoin address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The mktcoin address involved in the transaction\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending mktcoin\n"
    ...
    StartShutdown();
    return "wallet encrypted; MKTcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CMKTcoinAddress address(input.get_str());
            if (!address.IsValid())
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CMKTcoinAddress(address).ToString()));
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The mktcoin address to receive the change\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CMKTcoinAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid mktcoin address");
