...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the mktcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CMKTcoinAddress(CScriptID(script)).ToString()));
    }
