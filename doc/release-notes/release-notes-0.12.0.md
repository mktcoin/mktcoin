MKTcoin Core version 0.12.0 is now available from:

  <https://mktcoin.org/bin/mktcoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/MKTcoin-Qt (on Mac) or
mktcoind/mktcoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of MKTcoin Core or other software:
...
pre-0.12 versions of MKTcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start MKTcoin Core version 0.11 or
earlier.
...
ECDSA signatures inside MKTcoin transactions now use validation using
[libsecp256k1](https://github.com/mktcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/mktcoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of MKTcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of MKTcoin Core was to raise the default minimum
relay fee.

MKTcoin Core 0.12 will have a strict maximum size on the mempool. The
...
MKTcoin Core 0.12 also introduces new default policy limits on the length and
...
MKTcoin Core 0.12 nodes. MKTcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/mktcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in MKTcoin Core 0.12 does not yet have support for
...
This allows running mktcoind without having to do any manual configuration.
...
MKTcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. MKTcoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In MKTcoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in MKTcoin Core 0.13, and it is at this time undecided whether the
...
MKTcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
MKTcoin Core automatically creates a hidden service to listen on, without
manual configuration. MKTcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MKTcoin Core is listening, and
...
MKTcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals MKTcoin
Core to use floating fees. By default, MKTcoin Core will use floating
fees.
...
(default: `0.0002` MLM/kB).

At all times, MKTcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MLM.
Furthermore, MKTcoin Core will never create transactions paying less than
...
`mktcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/mktcoin/mktcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/mktcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/mktcoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /mktcoinrpc>
        ...
        # optional bypass mktcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< mktcoinrpc:<password>
    </Location>
...
Restarting mktcoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of MKTcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new mktcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include mktcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix mktcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of mktcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override MKTCOIND and MKTCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [mktcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add MKTCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libmktcoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "MLM" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/mktcoin/).
