MKTcoin Core version 0.11.0 is now available from:

  <https://mktcoin.org/bin/mktcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer MLM/kB fee to be
...
For example, add the following to `mktcoin.conf`:
...
related to the blockchain in the mktcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows MKTcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that MKTcoin Core will 
...
For an example script see [mktcoin-submittx](https://github.com/laanwj/mktcoin-submittx).
...
- #5334 `cf87536` libmktcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new mktcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only MKTcoinAmountField
- #5753 `0f44672` Add mktcoin logo to about screen
...
- #5809 `46bfbe7` Add mktcoin-cli man page
...
- #5849 `41259ca` contrib/init/mktcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename MKTcoin-Qt.app to MKTcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for mktcoin-tx)
