...
###   This script attempts to download the signature file SHA256SUMS.asc from mktcoin.org
...
WORKINGDIR="/tmp/mktcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://mktcoin.org/bin/"
VERSIONPREFIX="mktcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mktcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the MKTcoin Core binary release signing key installed?"
   fi
