#!/bin/bash
# Copyright (c) 2013 The MKTcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete MKTcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no MKTcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old MKTcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected MKTcoin 0.7 datadir."
    ;;
  3)
    echo "Detected MKTcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected MKTcoin 0.8 datadir."
    ;;
esac
